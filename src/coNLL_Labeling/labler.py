...
if __name__ == "__main__":
    # --- Define your paths (adjust as needed for your local setup) ---
    # Assuming 'output_tokens.txt' is in 'data/output_data/processed_for_labeling'
    base_output_dir = os.path.join(os.path.expanduser('~'), 'Documents', 'KAIM', 'week 4', 'EthioMart E-commerce Data Extractor', 'data', 'output_data')
    processed_for_labeling_dir = os.path.join(base_output_dir, 'processed_for_labeling')
    
    input_file = os.path.join(processed_for_labeling_dir, "output_tokens.txt")
    output_file = os.path.join(processed_for_labeling_dir, "my_labeled_data_conll.txt")
    
    os.makedirs(processed_for_labeling_dir, exist_ok=True) # Ensure output directory exists
